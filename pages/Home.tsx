...
import { ArrowRight, Sparkles, Wand2, Box, Layers, Terminal, Github, Coffee } from 'lucide-react';
import { Button, GlassCard, PageTransition } from '../components/UI';
        ...
        >
          <div className="border-t border-white/10 pt-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-8">
              {[
                { step: "01", title: "Deep Research", desc: "Validate market & tech." },
                { step: "02", title: "Define PRD", desc: "Scope features & user stories." },
                { step: "03", title: "Tech Design", desc: "Architect stack & DB schema." },
                { step: "04", title: "Agent Config", desc: "Generate Cursor/Claude rules." },
                { step: "05", title: "Build MVP", desc: "Execute with AI Agents." }
              ].map((s, i) => (
                <div key={i} className="group cursor-default">
                  <div className="flex items-center gap-4 mb-3">
                    <span className="flex items-center justify-center w-8 h-8 rounded-full bg-surface border border-white/10 text-xs font-mono text-primary-500 group-hover:bg-primary-500 group-hover:text-black transition-colors duration-300">
                      {s.step}
                    </span>
                    <strong className="text-slate-200 font-display text-lg tracking-tight">{s.title}</strong>
                  </div>
                  <p className="text-sm text-slate-500 pl-12 border-l border-white/5 group-hover:border-primary-500/30 transition-colors duration-300">{s.desc}</p>
                </div>
              ))}
            </div>
          </div>
