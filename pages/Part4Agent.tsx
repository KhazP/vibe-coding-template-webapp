...
import { Button, CopyBlock, Tooltip, PersonaError, StepNavigation } from '../components/UI';
import { GenerationVisualizer, PremiumEmptyState } from '../components/ArtifactSection';
...
const Part4Agent: React.FC = React.memo(() => {
  const { state, toggleTool, performGeminiAgent, setAgentOutputs, updateToolSettings, generationPhase, cancelGeneration } = useProject();
  ...
  const downloadAll = async () => {
    ...
    if (docs) {
      ...
      // Add Protocols (Unified with Part5Export)
      ...
      if (protocols) {
        ...
        tools.forEach(tId => {
          const protocol = getLaunchProtocol(tId, toolSettings);
          if (protocol) {
    ...
    // 3. Add README (Unified with Part5Export)
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid md:grid-cols-2 gap-8">
                    ...
                    {categoryTools.map(tool => {
                      ...
                      return (
                        <motion.div
                          ...
                          className={`cursor-pointer p-3.5 rounded-xl border transition-all relative group overflow-visible ${isSelected
                        ...
                        >
                          <div className="flex items-start gap-4 relative z-10">
                            {/* Checkbox Visual */}
                            <div className={`mt-0.5 w-5 h-5 rounded border flex items-center justify-center transition-all shrink-0 ${isSelected ? 'bg-primary-500 border-primary-500 shadow-sm' : 'border-slate-600 bg-slate-800/50 group-hover:border-slate-500'
                              }`}>
        ...
        <div className="space-y-4">
          {isGenerating ? (
            <GenerationVisualizer label={generationPhase || "Drafting Agent Configuration..."} onCancel={cancelGeneration} />
          ) : generated ? (
                ...
                <div className="flex items-center gap-2 overflow-x-auto custom-scrollbar pb-1">
                  {Object.keys(outputs).map(fileName => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-t-lg font-mono text-xs transition-colors whitespace-nowrap ${activeTab === fileName
            ...
            <PremiumEmptyState
              ...
              placeholder="Generate an AGENTS.md file to give all your AI tools a shared brain."
            />
          )}
        </div>
      </div>
