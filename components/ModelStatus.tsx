...
export const ModelStatus: React.FC = () => {
  const { state, setIsSettingsOpen, apiKey } = useProject();
  ...
  const contextUsagePercent = Math.min(100, (tokenCount / TOKEN_LIMITS.INPUT_GLOBAL) * 100);
  let healthColor = 'bg-emerald-500';
  let healthStatus = 'Healthy';
  
  if (tokenCount > TOKEN_LIMITS.CRITICAL_THRESHOLD) {
      healthColor = 'bg-red-500';
      healthStatus = 'Critical';
  } else if (tokenCount > TOKEN_LIMITS.WARNING_THRESHOLD) {
  ...
  return (
    <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-1 flex flex-wrap items-center gap-4 text-xs font-mono text-slate-400 mb-6 shadow-sm">
        ...
        <Tooltip content={`Current Context: ${tokenCount.toLocaleString()} / ${TOKEN_LIMITS.INPUT_GLOBAL.toLocaleString()} tokens. ${healthStatus === 'Heavy' ? 'High latency expected.' : ''}`}>
